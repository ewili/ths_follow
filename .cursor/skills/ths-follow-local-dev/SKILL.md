...
description: >-
  Start and configure ths_follow locally (signal-server :8000, follow-client
  :8100, Vue web build, portable python311, THS xiadan connect). Use when the
  user asks to 启动项目, run dev servers, connect 同花顺, or operate the Web/API
  without start.bat.
...
**先清日志**：按项目 Skill **`ths-follow-clear-logs-on-start`** 执行 `scripts/clear-logs.ps1`（启动/重启必做，再启 uvicorn）。
